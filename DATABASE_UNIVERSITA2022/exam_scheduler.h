...
//^(( +)?(s[\d]+)( +)?;( +)?([\w]+)( +)?;(( +)?([\w]+)( +)?);( +)?([a-zA-Z0-9.\-%]+)@([a-zA-Z0-9.\-%]+)\.([a-z]{2,3})( +)?)$
            ...
            if(!AvailabilityContent.empty()){
                ...
                for (auto it = AVData.begin();it!=AVData.end(); it++)
                {
                    if((*it)[0]==anno){
                        ...
                        for(const auto &x : AvailabilityAddContent){
    ...
    [[maybe_unused]] vector<string> GetAvailabilities(){
...
public:
   ...
   // Exam() = default;
    ...
    [[maybe_unused]] [[nodiscard]] int getVersion() const{
       return m_version;
   }
...
bool UnibaseErrors(const std::map<int,Student> &s,const std::map<int,Prof> &p,const std::map<int,Course> &c,const std::map<int,Major> &m,const std::map<int,Aula> &a){
//checks for all possible conflicts and errors in the database,  outputs true if none were found
