...
public:
    ...
    explicit Student(int mat = 0,const string& em = "",const string& nom = "",const string& cogn = ""){
        _matricola = mat;
        _email = em;
        _nome = nom;
        _cognome = cogn;
...
public:
    ...
    [[nodiscard]] string cognome() const{
        return m_cognome;
    }
...
public:
    ...
    Course(int key,const std::vector<string> &content,const std::map<int,Major> &m)
    {
        ...
        int enumber = (int)(content.size() - 5);
    ...
    [[nodiscard]] std::map<string,instance> GetInstanceList() const {
        return m_instances;
    }

    [[nodiscard]] std::pair<bool,instance> Instance (const string& year) const{
        if(isYear(year)){
...
//per instance!
    ...
    [[nodiscard]] bool active (const instance& in) const{
        return in.attivo;
    }

    [[nodiscard]] int nr_versions (const instance& in) const{
        return in.nr_versioni;
    }

    [[nodiscard]] vector<string> group (const instance& in) const{
        return in.raggrupamento;
    }

    [[nodiscard]] int prof_titolare (const instance& in) const{
       return stoi((((in.lista_versioni[0]).lista_prof)[0]).s_id);
    }

    [[nodiscard]] vector<string> lista_codici_vers (const instance& in) const{
        return in.codici_versioni;
    }

    [[nodiscard]] int exam_duration (const instance& in) const{
        return in.durata_esame;
    }

    [[nodiscard]] int entrance_time (const instance& in) const{
        return in.t_ingresso;
    }

    [[nodiscard]] int exit_time (const instance& in) const{
        return in.t_uscita;
    }

    [[nodiscard]] string mode (const instance& in) const{
        return in.modalita;
    }

    [[nodiscard]] string place (const instance& in) const{
        return in.luogo;
    }

    [[nodiscard]] int nr_booked (const instance& in) const{
        return in.nr_iscritti;
    }

    [[nodiscard]] string version_code (const instance& in,int vers) const{
    ...
    [[nodiscard]] int nr_versioni () const{
        return m_nr_versione;
    }
    [[nodiscard]] int ore_esercitazione () const{
        return m_ore_es;
    }
    [[nodiscard]] int ore_lab () const{
        return m_ore_lab;
    }
    [[nodiscard]] int ore_lezione () const{
        return m_ore_lez;
    }
    [[nodiscard]] string titolo () const{
        return m_titolo;
    }
    [[nodiscard]] int cfu () const{
        return m_cfu;
    }
    [[nodiscard]] int course_semester () const{
        return m_semester;
    }

    [[nodiscard]] int majorid() const{
        return m_majorid;
    }
};
...
public:
    ...
    explicit Aula(int id = 0,const string& cap = "", const string& cap_exam = "",const string& nm = "",const string& tp = "")
    {
        m_id = id;
        m_nome = nm;
        m_tipo = tp;
