...
public:
    ...
    [[maybe_unused]] void debug_Print_DB(){
        cout<<"\n######## SDBcontent ########\n";
    ...
    bool checkProfEmails(){
        ...
        for(it = PDBData.begin();it != PDBData.end();it++){
            ...
            for(ptr = PDBData.begin(); ptr != PDBData.end();ptr++) {
                if(ptr != it) {
                    if ((*it)[3] == (*ptr)[3]) {
                        int line_email1 = (int)distance(PDBData.begin(),it);
                        int line_email2 = (int)distance(PDBData.begin(),ptr);
    ...
    bool checkProfEmails_u(){
        ...
        for(it = Pbase.begin();it != Pbase.end();it++){
            ...
            for(ptr = Pbase.begin(); ptr != Pbase.end();ptr++) {
                if(ptr != it) {
                    if ((it->second)[2] == (ptr->second)[2]) {
                        int line_email1 = (int)distance(Pbase.begin(),it);
                        int line_email2 = (int)distance(Pbase.begin(),ptr);
...
public:
    ...
    [[maybe_unused]] void printCourseFile(){
                ...
                //
                   ...
                   for(tp=(it->second).begin();tp != (it->second).end();++tp){
                       if((*tp)==n[1]){
                           ptr = tp;
                          //F = *tp;////////////
                           location = (int)(ptr - (it->second).begin());
                           flag=1;
                       }
                   }
